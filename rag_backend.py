...
def generate_text_embeddings(prompt, output_embedding_length=3072):
    """Generate embeddings using OpenAI's text-embedding-3-large"""
    
    if not prompt:
        raise ValueError("Please provide a text prompt as input")
    ...
    try:
        ...
        # Ensure the embedding matches the requested length
        return embedding_vector[:output_embedding_length]
...
def load_or_initialize_stores():
    """Load FAISS index, stored items, and initialize BM25 for keyword-based search."""
    index = faiss.IndexFlatL2(3072)  # Adjust dimensions based on your embedding model
    all_items = []
    query_embeddings_cache = {}
    bm25_corpus = []  # New BM25 corpus storage
    ...
    if os.path.exists(os.path.join(VECTOR_STORE, ITEMS_PICKLE)):
        with open(os.path.join(VECTOR_STORE, ITEMS_PICKLE), 'rb') as f:
            ...
            bm25_corpus = [item['text'].split() for item in all_items if 'text' in item]  # Tokenize for BM25
    ...
    return index, all_items, query_embeddings_cache, bm25
...
def hybrid_search(query, index, all_items, bm25, top_k=10):
    """
    Perform hybrid retrieval using FAISS (semantic search) and BM25 (keyword search).
    Returns ranked results combining both scores.
    """
    # ✅ Step 1: Filter items that contain 'text'
    text_items = [item for item in all_items if 'text' in item]  # Only keep items with text

    # 1️⃣ **BM25 Keyword Search**
    bm25_scores = bm25.get_scores(query.split())  # Compute BM25 relevance scores
    bm25_ranked = np.argsort(bm25_scores)[::-1][:top_k]  # Get top BM25 results
    
    # ✅ Step 2: Ensure we only fetch items that have 'text'
    bm25_results = [text_items[idx] for idx in bm25_ranked if idx < len(text_items)]

    # 2️⃣ **FAISS Semantic Search**
    query_embedding = generate_text_embeddings(query)
    distances, faiss_results = index.search(np.array(query_embedding, dtype=np.float32).reshape(1,-1), top_k)

    # ✅ Step 3: Ensure we only fetch items that have 'text'
    faiss_results = [text_items[idx] for idx in faiss_results.flatten() if idx < len(text_items)]

    # 3️⃣ **Combine BM25 & FAISS Scores**
    combined_results = {}
    
    for item in bm25_results:
        if 'text' in item:  # ✅ Check before accessing
            combined_results[item['text']] = {'bm25_score': bm25_scores[text_items.index(item)], 'faiss_score': 0}

    for item in faiss_results:
        if 'text' in item:  # ✅ Check before accessing
            if item['text'] in combined_results:
                combined_results[item['text']]['faiss_score'] = 1  # Mark presence in FAISS results
            else:
                combined_results[item['text']] = {'bm25_score': 0, 'faiss_score': 1}

    # 4️⃣ **Sort Final Results Based on Combined Scores**
    ranked_results = sorted(
        combined_results.keys(),
        key=lambda x: (combined_results[x]['bm25_score'] + combined_results[x]['faiss_score']),
        reverse=True
    )

    # ✅ Step 4: Ensure we only return items that exist in 'text_items' and have 'text'
    return [item for item in text_items if item['text'] in ranked_results][:top_k]
