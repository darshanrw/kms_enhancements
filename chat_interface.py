...
def main():
    ...
    index, all_items, query_embeddings_cache, bm25 = load_or_initialize_stores()
            ...
            with st.spinner("Thinking..."):
                if all_items:
                    ...
                    if query in query_embeddings_cache:
                        ...
                        query_embedding = generate_text_embeddings(prompt=query)
                    ...
                    matched_items = hybrid_search(query, index, all_items, bm25, top_k=10)

                    # Extract matched items for GPT processing
                    retrieved_texts = [item['text'] for item in matched_items if 'text' in item]

                    # Get matched items
                    matched_items = [{k: v for k, v in item.items() if k != 'embedding'} for item in matched_items]
